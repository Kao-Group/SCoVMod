...
import static scovmod.model.state.infection.InfectionState.RECOVERED_ELDERLY;

public class RecoveredElderlyLocations implements Cache {

    IntSet locations = new IntOpenHashSet();

    public void wholeLocationUpdate(int locationId, LocalPopulation population) {
        if (!population.getAllInState(RECOVERED_ELDERLY).isEmpty()) {

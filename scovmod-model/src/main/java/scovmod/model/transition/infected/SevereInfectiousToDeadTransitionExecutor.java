...
public class SevereInfectiousToDeadTransitionExecutor {
    ...
    public SevereInfectiousToDeadTransitionExecutor(
            StateModifier sm,
            Random rnd,
            double timeStep,
            Parameters params,
    ...
    public void apply(int personID) {
        ...
        double deathRateModPerHB = dr.getDeathRateModifierBasedOnHealthIndex(personID);
            ...
            switch(ageClass){
                case YOUNG:
                    if (rnd.nextPoissonReturnsOneOrMore(params.getSiToD_YoungRate() * deathRateModPerHB * timeStep)) {
                        sm.updateInfectionState(personID, DEAD_YOUNG);
                    }
                    break;
                case ADULT:
                    if (rnd.nextPoissonReturnsOneOrMore(params.getSiToD_AdultRate() * deathRateModPerHB * timeStep)) {
                        sm.updateInfectionState(personID, DEAD_ADULT);
                    }
                    break;
                case ELDERLY:
                    if (rnd.nextPoissonReturnsOneOrMore(params.getSiToD_ElderlyRate() * deathRateModPerHB * timeStep)) {

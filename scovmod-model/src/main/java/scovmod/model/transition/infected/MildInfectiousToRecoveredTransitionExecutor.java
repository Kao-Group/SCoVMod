...
public class MildInfectiousToRecoveredTransitionExecutor {
    ...
    public MildInfectiousToRecoveredTransitionExecutor(
            StateModifier sm,
            Random rnd,
            double timeStep,
            Parameters params,
    ...
    public void apply(int personID) {
        AgeClass ageClass = peopleAgeClasses.getOrDefault(personID,AgeClass.YOUNG);
            ...
            switch(ageClass){
                case YOUNG:
                    if (rnd.nextPoissonReturnsOneOrMore(params.getMiToR_YoungRate() * timeStep)) {
                        sm.updateInfectionState(personID, RECOVERED_YOUNG);
                    }
                    break;
                case ADULT:
                    if (rnd.nextPoissonReturnsOneOrMore(params.getMiToR_AdultRate() * timeStep)) {
                        sm.updateInfectionState(personID, RECOVERED_ADULT);
                    }
                    break;
                case ELDERLY:
                    if (rnd.nextPoissonReturnsOneOrMore(params.getMiToR_ElderlyRate() * timeStep)) {

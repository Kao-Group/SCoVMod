...
public class HospitalisedToRecoveredTransitionExecutor {
    ...
    public HospitalisedToRecoveredTransitionExecutor(
            StateModifier sm,
            Random rnd,
            double timeStep,
            Parameters params,
    ...
    public void apply(int personID) {
        AgeClass ageClass = peopleAgeClasses.getOrDefault(personID,AgeClass.YOUNG);
            ...
            switch(ageClass){
                case YOUNG:
                    if (rnd.nextPoissonReturnsOneOrMore(params.gethToR_YoungRate() * timeStep)) {
                        sm.updateInfectionState(personID, RECOVERED_YOUNG);
                    }
                    break;
                case ADULT:
                    if (rnd.nextPoissonReturnsOneOrMore(params.gethToR_AdultRate() * timeStep)) {
                        sm.updateInfectionState(personID, RECOVERED_ADULT);
                    }
                    break;
                case ELDERLY:
                    if (rnd.nextPoissonReturnsOneOrMore(params.gethToR_ElderlyRate() * timeStep)) {

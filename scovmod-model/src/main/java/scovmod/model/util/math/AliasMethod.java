...
public class AliasMethod {
    ...
    // Leaving alias and probability null for now
    // So that they can be mocked and help with unit tests
    public AliasMethod() {
        /* Store the underlying generator. */
        //TODO use our own random
        this.random = new java.util.Random();
        this.alias = null;
        this.probability = null;
    }

    public void initialise(List<Double> probabilities) {
        if (alreadyInitialised) {
            throw new UnsupportedOperationException("Cannot initialise alias method more than once");
        } else {
            ...
            if (sum > 1 + 1e-10 || sum < 1 - 1e-10) {
                throw new IllegalArgumentException("Probability vector must sum to 1. Sum is: "+sum);
            }

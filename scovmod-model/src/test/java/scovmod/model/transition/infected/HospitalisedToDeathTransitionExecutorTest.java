...
public class HospitalisedToDeathTransitionExecutorTest {

    private static int PERSON_1 = 100;
    private static int TIMESTEP = 1;
    private static double H_TO_D_RATE = 0.3;
    ...
    public void youngPerson_CAN_Transition(){
        ...
        when(params.gethToD_YoungRate()).thenReturn(H_TO_D_RATE);
        when(dr.getDeathRateModifierBasedOnHealthIndex(PERSON_1)).thenReturn(DEATH_RATE_MODIFICATION);

        HospitalisedToDeadTransitionExecutor instance = new HospitalisedToDeadTransitionExecutor(sm, rnd, TIMESTEP, params,slaac, dr);

        when(rnd.nextPoissonReturnsOneOrMore(H_TO_D_RATE * DEATH_RATE_MODIFICATION *  TIMESTEP))
    ...
    public void adultPerson_CAN_Transition(){
        ...
        when(slaac.getPeopleAgeClasses()).thenReturn(peopleAgeClasses);
        when(params.gethToD_AdultRate()).thenReturn(H_TO_D_RATE);
        when(dr.getDeathRateModifierBasedOnHealthIndex(PERSON_1)).thenReturn(DEATH_RATE_MODIFICATION);
    ...
    public void elderlyPerson_CAN_Transition(){
        ...
        when(slaac.getPeopleAgeClasses()).thenReturn(peopleAgeClasses);
        when(params.gethToD_ElderlyRate()).thenReturn(H_TO_D_RATE);
        when(dr.getDeathRateModifierBasedOnHealthIndex(PERSON_1)).thenReturn(DEATH_RATE_MODIFICATION);

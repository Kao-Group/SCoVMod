...
public class ExposedToRecoveredTransitionExecutorTest {

    private static int PERSON_1 = 100;
    private static int TIMESTEP = 1;
    private static double E_TO_R_RATE = 0.3;
    ...
    public void youngPerson_CAN_Transition(){
        ...
        when(params.geteToR_YoungRate()).thenReturn(E_TO_R_RATE);

        ExposedToRecoveredTransitionExecutor instance = new ExposedToRecoveredTransitionExecutor(sm, rnd, TIMESTEP, params,slaac);

        when(rnd.nextPoissonReturnsOneOrMore(E_TO_R_RATE * TIMESTEP))
    ...
    public void adultPerson_CAN_Transition(){
        ...
        when(slaac.getPeopleAgeClasses()).thenReturn(peopleAgeClasses);
        when(params.geteToR_AdultRate()).thenReturn(E_TO_R_RATE);
    ...
    public void elderlyPerson_CAN_Transition(){
        ...
        when(slaac.getPeopleAgeClasses()).thenReturn(peopleAgeClasses);
        when(params.geteToR_ElderlyRate()).thenReturn(E_TO_R_RATE);

...
public class MildInfectiousYoungLocationsTest {
	...
	public void updatingByWholeLocationStatus(){
		MildInfectiousYoungLocations instance = new MildInfectiousYoungLocations();
		...
		assertEquals(NONE, instance.getMildInfectiousYoungLocations());

		FakeLocalPopulation location2pop = new FakeLocalPopulation();
		location2pop.testSet(PERSON_5, MILD_INFECTIOUS_YOUNG);
		instance.wholeLocationUpdate(LOCATION_2, location2pop);

		assertEquals(setOf(LOCATION_2), instance.getMildInfectiousYoungLocations());

		FakeLocalPopulation location1updatedPop = new FakeLocalPopulation();
		location1updatedPop.testSet(PERSON_1, SUSCEPTIBLE);
		location1updatedPop.testSet(PERSON_3, MILD_INFECTIOUS_YOUNG);
		instance.wholeLocationUpdate(LOCATION_1, location1updatedPop);

		assertEquals(setOf(LOCATION_2, LOCATION_1), instance.getMildInfectiousYoungLocations());
	}
	...
	public void edgeCaseUpdatesDoNothing(){
		...
		FakeLocalPopulation location1pop = new FakeLocalPopulation();
		location1pop.testSet(PERSON_1, MILD_INFECTIOUS_YOUNG);
		location1pop.testSet(PERSON_2, EXPOSED_ELDERLY);
		location1pop.testSet(PERSON_3, EXPOSED_ADULT);
		instance.wholeLocationUpdate(LOCATION_1, location1pop);
		...
		assertEquals(setOf(LOCATION_1), instance.getMildInfectiousYoungLocations());
	}
